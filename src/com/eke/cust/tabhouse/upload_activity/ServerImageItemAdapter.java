...
public class ServerImageItemAdapter extends BaseAdapter {
    private Context context;
    private List<PhoteBean> list;
    private LayoutInflater layoutInflater;
    private GridView gridView;

    public ServerImageItemAdapter(List<PhoteBean> list, Context context, GridView gridView) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        Holder holder;
        if (convertView == null) {
            convertView = layoutInflater.inflate(R.layout.album_server_images_item, parent, false);
            holder = new Holder();
        ...
        if (!StringCheckHelper.isEmpty(path)) {
            LoadImageUtil.getInstance().displayImage(ServerUrl.buildImageUrl(path), holder.imageView, R.drawable.house_source);

        } else {
            holder.imageView.setImageResource(R.drawable.house_source);
        }
