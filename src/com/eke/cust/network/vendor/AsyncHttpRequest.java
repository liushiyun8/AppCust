...
public class AsyncHttpRequest implements Runnable {
	...
	private boolean isRequestPreProcessed = false;
	private boolean isNetworkUnAvaible = false;
	...
	public void run() {
        ...
        if(isNetworkUnAvaible) {
        	isFinished = true;
        	responseHandler.sendFailureMessage(0, null, null, new ConnectTimeoutException("network is unavaible") );
        	return;
        }
		if (isCancelled()) {
			return;
		...
		try {
			...
			if (!isCancelled()) {
				...
				//Log.e(TAG, "makeRequestWithRetries returned error", e);
			}
		}
		...
		try {
			while (retry) {
				try {
					...
					//LogUtil.i(TAG, "NullPointerException");
				...
				if(isTimeOut) {
					responseHandler.sendFailureMessage(0, null, null, new ConnectTimeoutException("ConnectTimeout"));
				}
				if (retry) {
