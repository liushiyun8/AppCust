...
public class GifDrawable extends Drawable implements Animatable,
	 ...
	 */
	public GifDrawable(@Nullable ContentResolver resolver, @NonNull Uri uri)
			throws IOException {
	...
	void startAnimation(long lastFrameRemainder) {
		if (mIsRenderingTriggeredOnDraw) {
			...
			mInvalidationHandler.sendEmptyMessageAtTime(MSG_TYPE_INVALIDATION,
					0);
		} else {
			waitForPendingRenderTask();
			mSchedule = mExecutor.schedule(mRenderTask,
					Math.max(lastFrameRemainder, 0), TimeUnit.MILLISECONDS);
		}
	}
	...
	public void setSpeed(
			@FloatRange(from = 0, fromInclusive = false) final float factor) {
	 ...
	 * If <code>position</code> is greater than duration of the loop of
	 * animation (or whole animation if there is no loop) then animation will be
	 * sought to the end, no exception will be thrown.<br>
	 ...
	 * <code>frameIndex</code> exceeds number of frames, seek stops at the end,
	 * no exception is thrown.
	 * 
	 * @param frameIndex
	 *            index of the frame to seek to (zero based)
	 * @throws IllegalArgumentException
	 *             if <code>frameIndex</code>&lt;0
	 */
	public void seekToFrame(
	...
	protected void onBoundsChange(Rect bounds) {
		...
		if (shader != null) {
			...
			shaderMatrix.preScale(mDstRectF.width() / mBuffer.getWidth(),
	...
	public void draw(Canvas canvas) {
		...
		if (mPaint.getShader() == null) {
			...
			canvas.drawRoundRect(mDstRectF, mCornerRadius, mCornerRadius,
					mPaint);
		}
		if (clearColorFilter) {
		...
		if (mIsRenderingTriggeredOnDraw && mIsRunning
			...
			mSchedule = mExecutor.schedule(mRenderTask, renderDelay,
	...
	public void setCornerRadius(@FloatRange(from = 0) final float cornerRadius) {
		...
		if (cornerRadius > 0) {
			bitmapShader = new BitmapShader(mBuffer, Shader.TileMode.CLAMP,
