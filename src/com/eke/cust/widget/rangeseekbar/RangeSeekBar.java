...
package com.eke.cust.widget.rangeseekbar;
...
public class RangeSeekBar<T extends Number> extends ImageView {
    ...
    private void init(Context context, AttributeSet attrs) {
        float barHeight;
        int thumbNormal = R.drawable.seek_thumb_pressed_bg;
        int thumbPressed = R.drawable.seek_thumb_bg;
        int thumbDisabled = R.drawable.seek_thumb_pressed_bg;
        int thumbShadowColor;
        int defaultShadowColor = Color.argb(75, 0, 0, 0);
        int defaultShadowYOffset = DensityUtil.dpToPx(context, 2);
        int defaultShadowXOffset = DensityUtil.dpToPx(context, 0);
        int defaultShadowBlur = DensityUtil.dpToPx(context, 2);

        if (attrs == null) {
            setRangeToDefaultValues();
            internalPad = DensityUtil.dpToPx(context, INITIAL_PADDING_IN_DP);
            barHeight = DensityUtil.dpToPx(context, LINE_HEIGHT_IN_DP);
        ...
        textSize = DensityUtil.dpToPx(context, DEFAULT_TEXT_SIZE_IN_DP);
        distanceToTop = DensityUtil.dpToPx(context, DEFAULT_TEXT_DISTANCE_TO_TOP_IN_DP);
        textOffset = !showTextAboveThumbs ? 0 : this.textSize + DensityUtil.dpToPx(context,
    ...
    protected synchronized void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int height = thumbImage.getHeight()
                + (!showTextAboveThumbs ? 0 : DensityUtil.dpToPx(getContext(), HEIGHT_IN_DP))
...
//            canvas.drawText(maxLabel, getWidth() - minMaxLabelSize, minMaxHeight, paint);
        ...
        if (showTextAboveThumbs && (activateOnDefaultValues || !selectedValuesAreDefault)) {
            ...
            if (!singleThumb) {
                ...
                int spacing = DensityUtil.dpToPx(getContext(), TEXT_LATERAL_PADDING_IN_DP);
