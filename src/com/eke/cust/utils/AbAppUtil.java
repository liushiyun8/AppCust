...
public class AbAppUtil {
	...
	/**
	 * 获取 manifest 里面的 data 数据
	 * 
	 * @return
	 */
	public static int getIntMetaData(Context mContext, String key) {
		try {
			if (appInfo == null) {
				appInfo = mContext.getPackageManager().getApplicationInfo(mContext.getPackageName(), PackageManager.GET_CONFIGURATIONS | PackageManager.GET_META_DATA);
			}
	 ...
	 * @return
	 */
	public static String getStringMetaData(Context mContext, String key) {
		try {
			if (appInfo == null) {
				appInfo = mContext.getPackageManager().getApplicationInfo(mContext.getPackageName(), PackageManager.GET_CONFIGURATIONS | PackageManager.GET_META_DATA);
			}
	 ...
	 * 
	 * @Description (拨打电话)
	 * @CreateTime 2015-7-7下午3:17:42
	 * @Author john
	 * @param mContext
	 * @param phoneNumber
	 * @param shopid
	 */
	public static void callPhone(Context mContext, String phoneNumber, String shopid) {
		// startEvent(mContext, "DH01", "", "电话统计");
		// startEvent(shopid);
		Intent intent = new Intent();
