 ...
 * 列表Activity
 */
public abstract class BaseRecyclerViewActivity<T> extends BaseActivity implements SuperSwipeRefreshLayout.OnRefreshListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mRecyclerView.addOnScrollListener(new RecyclerView.OnScrollListener() {
			...
			public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
				...
				//  _refreshLayout.setEnabled(topRowVerticalPosition >= 0);
	...
	protected EmptyRecyclerView createListView() {
		return new EmptyRecyclerView(this);
	}
	...
	private void updateData(final boolean showLoading) {
		...
		ThreadUtils.runOnWorkThread(new Runnable() {
			...
			public void run() {
				...
				Log.d("time", "start_time=" + System.currentTimeMillis());
				final ArrayList<T> ret = loadListData();
				ThreadUtils.runOnUiThread(new Runnable() {
					...
					public void run() {
						...
						reloadData(result, ret != null ? ret.size() : -1);
						hideLoading();
	...
	private void reloadData(ArrayList<T> dataSource, int moreDataSize) {

//        // 显示数据加载结果
//        if (_RefreshState == RefreshState.LS_Refresh) {
//            // 下拉刷新
//            showRefreshResult();
//        } else {
//            // 上拉加载更多
//            showLoadMoreResult(moreDataSize);
//        }
		...
		if (dataSource != null) {
			if (_RefreshState == RefreshState.LS_Refresh || _RefreshState == RefreshState.LS_INIT) {
				...
				//下拉刷新
				_adapter = new ListViewAdapter(mContext, dataSource);
	...
	public abstract ArrayList<T> loadListData();
